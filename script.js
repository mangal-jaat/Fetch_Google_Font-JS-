/// Define api component as variable ///
var api_key = "AIzaSyCp5_6h6GXqnWXt1c4ffW34LWk7C0hm45k";
var filter = "popularity", popularity = "popularity", alfhabeticali = "alpha", NewItemFirst = "data", topStyle = "style",trending ="trending";
var api = `https://www.googleapis.com/webfonts/v1/webfonts?key=${api_key}&sort=${filter}`//&sort=${trending}`;

/// Required variable for pagination */
const spinner = document.querySelector('.js-spinner')
let item = 15;
let currPage = 1;

/** Fetch Google Fonts (async) 
/** ========================== */
const fonts = async () => {
...
fonts().then(data => {
  ...
  result = data.items.slice(item*currPage - item, item*currPage);
  setTimeout(() => { displayFont(result) }, "3000");
}) 
//////////////////////////////////////////

/** Define Search Font Function 
/** ========================== */
const searchFont = () => {
  fonts().then(data => {
    spinner.classList.remove('hidden');
...
const paginationFont = (go) => {
  clearFont();
  if(go == "next"){currPage += 1} 
  else if(go == "prev"){currPage -= 1}
  searchFont();
}
/** ========================== */
...
const displayFont = (data) => {
  data.forEach(font => {
    ...
    document.querySelectorAll('#resultList pre code').forEach((el) => { hljs.highlightElement(el);});
...
/// download message
const downloadedMsg = () =>{
  $message.success("(All in One .ttf) File Downloaded Successfully")
}

/// filter font by search query
jQuery(".search").on("keyup", function() {
  var keyword = $(this).val().toLowerCase();
  $("body #resultList .card").filter(function() {
    $(this).toggle($(this).text().toLowerCase().indexOf(keyword) > -1)
  });
});

//// copy target by "highlight" class
document.body.addEventListener('click', function(e){
  var target = e.target || e.srcElement;
  var x = target.className.indexOf("highlight") !== -1;
  var y = target.parentNode.className.indexOf("highlight") !== -1;
  if (x || y){
    var range, selection;
...
/// filter font
const filterFont = (event,go) => {
  document.querySelector(".filter-btn.active").classList.remove("active");
  event.target.classList.add("active");
  clearFont();
  if(go == "popular"){api = `https://www.googleapis.com/webfonts/v1/webfonts?key=${api_key}&sort=${popularity}`;}
  else if(go == "alpha"){api = `https://www.googleapis.com/webfonts/v1/webfonts?key=${api_key}&sort=${alfhabeticali}`}
  else if(go == "style"){api = `https://www.googleapis.com/webfonts/v1/webfonts?key=${api_key}&sort=${topStyle}`}
  else if(go == "trending"){api = `https://www.googleapis.com/webfonts/v1/webfonts?key=${api_key}&sort=${trending}`}
  searchFont();
  $message.success("Font Sorted By "+go)
}




$(function() { // filter-dropdown toggle
    $('.filter-toggle').click(function() { $(this).next('.filter-dropdown').slideToggle();
    });
    
    $(document).click(function(e) 
    { 
    var target = e.target; 
    if (!$(target).is('.filter-toggle') && !$(target).parents().is('.filter-toggle')) 
